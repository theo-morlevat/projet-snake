...
if [ $1 == --input ] || [ $1 == -i ]; then  #option input, détaille les arguments d'entrée pour le script.
...
if [ $1 == --output ] || [ $1 == -o ]; then  #option output, donne une explication des données de sortie.
...
lastextension="${2##*.}" #On stock la dernière extension du nom du fichier dans la variable lastextension
...
if [ -z "$2" ] || [ ! -e "$2" ] || [ ! -f "$2" ] || [[ ! -s "$2" ]] || [ $lastextension != sam ]; then #On vérifie s'il n'y a pas d'érreur du à: un argument manquant, un fichier absent, un argument ne correspondant pas à un fichier régulier, un fichier vide ou n'ayant pas l'extension .sam.
        ...
        echo -e "$error" #On affiche les différentes erreurs liées à l'exécution de la commande.
        exit 1 #S'il y a une erreur alors le programme s'arrête
else
        echo "Verifications completed" #Sinon on averti l'utilisateur que l'on va pouvoir travailler sur le fichier.
fi


echo "This script could work on all data or on specifics needs. You want to work on : 'full', 'cigar', 'mapping', 'fasta'" # affiche un message demandant l'analyse à réaliser
read parameter # l'utilisateur entre la valeur dans la console, la valeur est stockée dans la variable parameter
